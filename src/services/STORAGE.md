...
## Rules
- Components read through `getSessions()` / `getExerciseDefinitions()` and write through `saveSession`, `deleteSession`, `saveExerciseDefinitions` or `clearAllSessions`. Never call `/api/*` or localStorage directly.
