...
### Added
- Terminal CLI theme: phosphor green on black, VT323 headings and JetBrains Mono, dashed borders and no animation. Dialogs open as full-screen panes titled `-- PLATES --` and closed with Esc, badges read `[+W]`, `[+R]`, `[DL]` and `[-W]`, done sets read `[x]`, progress reads `[######----] 60%`, the plate calculator draws `|==[10][15][20]--- 100 kg ---[20][15][10]==|`, and the nav bar is a status line with `sqlite:ok pending:0`. The fonts are bundled.
- A command line for the tracker, on in Terminal CLI: `62.5x8@2` logs load, reps and RIR on the next open set and marks it done; `x9`, `65x`, `+`, `-`, `d`, `u`, `n`, `p`, `rest 120`, `skip`, `plates`, `note …`, `fin` and `q` cover the rest, and `?` lists them. Up and Down recall earlier commands. Keys: `/` or `:` focuses the prompt, `j` and `k` move a set cursor, Space ticks the set under it, 1, 2 and 3 switch tabs, and `s` starts the next workout.
- Mechanical Telemetry theme: slate, cold cyan and amber, Chakra Petch labels, IBM Plex Sans text and IBM Plex Mono figures, panels on a faint instrument grid and cards with corner brackets. Each exercise shows readouts (last load and reps, target, the change in load, estimated 1RM) and a sparkline of its recent volume. Each barbell, dumbbell and landmine set shows the plates on one end and how much of your stock they take ("15 kg 2/2"). The RIR pick gets a half-dial gauge, the rest timer a 270° dial, and calendar days glow amber by volume relative to the month's heaviest day. The fonts are bundled.
- Optional reps in reserve (RIR) per set: once a set is marked done, a 0–5 picker appears under it, and history shows the set as "8 @ RIR 2". It stays blank unless you pick a value, and picking it again clears it. RIR is saved in the existing `rpe` field as 10 − RIR, so sessions and backups keep their format.
- Progress charts: for the chosen exercise, one chart of the estimated 1RM and one of the session volume (weight × reps over its completed sets), session by session. Each is a hand-drawn SVG line with the latest session marked; hovering, or focusing a chart and pressing the arrow keys, shows a session's date and value. Two charts rather than one with two scales, because the measures differ by a factor of about 20. The session list below stays as the table view.
